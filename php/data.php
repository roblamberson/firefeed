...
-'_________`-____________'__ \  (O)  (O)  / __`____________-'________`-     
 ...
 - rowCount is curently broken and must be rewritten
	+ this includes following inserts and updates too, broken af
 - regexps need looked over by someone more literate with them
 - some InvalidArgumentException are removed pending better option
	for fallback once the excessive leftovers are gone and rewrite happens
 */

class Raw { public $map; public $value; }

class Client {
	
	public $ip_address;
	
	public $browse_OS_vers;
	
	public $srv_ra;				public $srv_str;
	
	public function __construct(){
		
		$this->ip_address = $_SERVER['REMOTE_ADDR'];
		
		$this->browse_OS_vers = $_SERVER['HTTP_USER_AGENT'];
		
		$this->serv_ra = array(); 
		
		foreach( $_SERVER as $key => $value ){
			
			//$this->srv_ra[$$key] = $value;
			
			$this->srv_str .= ' $_SERVER["'.$key.'"] = '.$value."; |";
...
class _profane_abstraction_ {
	
	public $pdo;				protected $type;	protected $prefix;

	protected $statement;		protected $dsn;		protected $logs = [];

	protected $logging = false;						protected $debug_mode = false;
	...
	public function __construct( array $options ){
		...
		// Just use standard quoted identifiers
		// if this is even needed w/ mssql gone
...
//				throw new InvalidArgumentException('Invalid PDO object supplied');
		...
		if( isset( $options[ 'dsn' ] ) ){
			...
			}else{ throw new InvalidArgumentException('Invalid DSN option supplied'); }
			...
			switch( $this->type ){
				...
				break;	
				/* Make a default mirror mysql casing */
		...
		//if( !in_array( $driver, PDO::getAvailableDrivers() ) ){
			...
			//throw new InvalidArgumentException("PDO driver not supported: {$driver}");
		...
		foreach( $attr as $key => $value){ $stack[] = is_int($key) ? $value : $key . '=' . $value; }

		$dsn = $driver . ':' . implode( $stack, ';' );

		if( in_array( $this->type, ['mysql', 'pgsql']) && isset( $options[ 'charset' ] ) ){
			
	$commands[] = "SET NAMES '{$options[ 'charset' ]}'" . ( $this->type === 'mysql' && isset($options[ 'collation' ]) ? " COLLATE '{$options[ 'collation' ]}'" : '' );
	...
	public function exec( $query, $map = [] ){
		...
		if( $this->logging ){ $this->logs[] = [$query, $map];
		
		}else{ $this->logs = [[$query, $map]]; }
	...
	protected function generate( $query, $map ){ $identifier = ['mysql' => '`$1`', 'mssql' => '[$1]']; // not currently including mssql funcs otherwise
// preg_replace('/"([a-zA-Z0-9_]+)"/i', isset($identifier[ $this->type ]) ?  $identifier[ $this->type ] : '"$1"', $query );
/* ^trash */
		$query = preg_replace('/"([a-zA-Z0-9_]+)"/i', isset($identifier[ $this->type ]) ?  $identifier[ $this->type ] : '"$1"', $query );

		foreach( $map as $key => $value ){
			
			if( $value[ 1 ] === PDO::PARAM_STR ){			$replace = $this->quote($value[ 0 ]);
				
			}else if( $value[ 1 ] === PDO::PARAM_NULL ){	$replace = 'NULL';
				
			}else if( $value[ 1 ] === PDO::PARAM_LOB ){	$replace = '{LOB_DATA}';
				
			}else{ $replace = $value[ 0 ]; }
	...
	public static function raw( $string, $map = [] ){ // renamed already
	// all old references call raw() now
		$raw = new Raw();	$raw->map = $map;	$raw->value = $string;	return $raw;
		
	}

	protected function isRaw( $object ){ return $object instanceof Raw; }

	protected function buildRaw( $raw, &$map ){
		
		if( !$this->isRaw( $raw ) ){ return false; }
		// 
//preg_replace_callback('/((FROM|TABLE|INTO|UPDATE)\s*)?\<([a-zA-Z0-9_\.]+)\>/i',
		$query = preg_replace_callback('/((FROM|TABLE|INTO|UPDATE)\s*)?\<([a-zA-Z0-9_\.]+)\>/i',
		function( $matches ){
	...
	protected function tableQuote( $table ){ return '"' . $this->prefix . $table . '"'; }

	protected function mapKey( ){ return ':OroPro_' . $this->guid++ . '_NobisLUCiFeR'; }

	protected function typeMap($value, $type){
		
		$map = [	'NULL' => PDO::PARAM_NULL,		'integer' => PDO::PARAM_INT,
					'double' => PDO::PARAM_STR,		'boolean' => PDO::PARAM_BOOL,
					'string' => PDO::PARAM_STR,		'object' => PDO::PARAM_STR,
	...
	protected function columnPush( &$columns, &$map ){
		...
		foreach( $columns as $key => $value ){
			...
			if( is_array( $value ) ){	$stack[] = $this->columnPush($value, $map);	
...
//	preg_match('/(?<col>[a-zA-Z0-9_\.]+) (?:\s*\((?<als>[a-zA-Z0-9_]+)\))?(?:\s*\[(?<type>(?:String|Bool|Int|Number|Object|JSON))\])?/i', $value, $match);
...
//				^ encap these bitches
				if( !empty( $match[ 'alias' ] ) ){
					...
					if( !empty( $match[ 'type' ] ) ){	$columns[ $key ] .= ' [' . $match[ 'type' ] . ']';	}

				}else{	$stack[] = $this->columnQuote( $match[ 'column' ] );	}
	...
	protected function arrayQuote( $array ){	$stack = [];
	...
	protected function innerConjunct( $data, $map, $conjunctor, $outer ){	$stack = [];
		...
		return implode( $outer . ' ', $stack );
		
	}

	protected function dataImplode( $data, &$map, $conjunctor ){	$stack = [];
		...
		foreach( $data as $key => $value ){
			...
			if( $type === 'array' && preg_match("/^(AND|OR)(\s+#.*)?$/", $key, $relation_match ) ){
				...
				$stack[] = $value !== array_keys(array_keys($value)) ? '(' . $this->dataImplode($value, $map, ' ' . $relationship) . ')' : '(' . $this->innerConjunct($value, $map, ' ' . $relationship, $conjunctor) . ')';
...
// 	preg_match('/(\[(?<join>\<\>?|\>\<?)\])?(?<table>[a-zA-Z0-9_]+)\s?(\((?<alias>[a-zA-Z0-9_]+)\))?/', $sub, $ma);
				...
				if( isset( $match[ 'operator' ] ) ){
					...
					if( in_array( $operator, ['>', '>=', '<', '<='] ) ){
						...
						switch( $type ){
							...
							// $map[ $map_key ] = $this->typeMap( $val, $type );
						...
						foreach( $value as $index => $item ){
							...
							$item = strval($item);
/* !: shit things trash shitty fucking regexp has to be copied from somewhere else
							if( preg_match('/(\[.+\]|_|%.+|.+)/', $item ) ){
								
								// regexpt won't work if I write it
	...
	protected function whereClause( $where, &$map ){	$where_clause = '';
		...
		if( is_array( $where ) ){
			...
			if( isset( $where[ 'MATCH' ] ) && $this->type === 'mysql' ){
				...
				if( is_array( $MATCH ) && isset($MATCH[ 'columns' ], $MATCH[ 'keyword' ] ) ){
					...
					$mode_array = ['natural' => 'IN NATURAL LANGUAGE MODE',
			...
			if( isset( $where[ 'GROUP' ] ) ){	$GROUP = $where[ 'GROUP' ];
			...
			if( isset( $where[ 'ORDER' ] ) ){
				...
				if( is_array( $ORDER ) ){	$stack = [];
					...
					foreach( $ORDER as $column => $value ){
						...
						}else if( is_int( $column ) ){ $stack[] = $this->columnQuote($value); }
				...
				}else{ $where_clause .= ' ORDER BY ' . $this->columnQuote($ORDER); }
...
/*
!is_numeric( $limit )
if( /* is_numeric( $LIMIT ) ){ $where_clause .= ' LIMIT ' . $LIMIT;

}else if( is_array($LIMIT) && is_numeric($LIMIT[ 0 ]) && is_numeric( $LIMIT[ 1 ] ) ){

	$where_clause = $where_clause . ' LIMIT ' . $LIMIT[ 1 ] . ' OFFSET ' . $LIMIT[ 0 ];
...
// 	preg_match('/(\[(?<join>\<\>?|\>\<?)\])?(?<table>[a-zA-Z0-9_]+)\s?(\((?<alias>[a-zA-Z0-9_]+)\))?/', $sub, $ma);
	...
	$joins[] = ( strpos( $key, '.' ) > 0 ? $this->columnQuote($key) : $table . '."' . $key . '"') . ' = ' . $this->tableQuote( isset( $match[ 'alias' ] ) ? $match[ 'alias' ] : $match[ 'table' ] ) . '."' . $value . '"';
...
//								$this->tableQuote(isset($match[ 'alias' ]) ? $match[ 'alias' ] : $match[ 'table' ]) . '."' . $value . '"';
			...
			if( is_null( $columns ) ){
				...
				if( !is_null( $where ) || ( is_array( $join ) && isset( $column_fn ) ) ){
						...
						$where = $join; $columns = null;
				
				}else{	$where = null; $columns = $join;	}
		...
		if( isset( $column_fn ) ){
			...
			if( $column_fn === 1 ){			$column = '1';
				// is_null( $where )
				if( is_null( $where ) ){	$where = $columns;	 }
...
// !empty( $key_match[ 'alias' ]) ? $key_match[ 'alias' ] : $key_match['col'];
//  
				$column_key = !empty( $key_match[ 'alias' ]) ? $key_match[ 'alias' ] : $key_match[ 'column' ];
				...
				}else{	$stack[ $value ] = [$column_key, 'String'];	}
...
//preg_match('/([a-zA-Z0-9_]+\.)?(?<column>[a-zA-Z0-9_]+)(\s*\[(?<type>(String|Bool|Int|Number))\])?/i', $key, $key_match);
				...
				}else{	$stack[ $key ] = [$column_key, 'String'];	}
		...
		foreach( $columns as $key => $value ){
			...
			if( is_int( $key ) || $isRaw ){
				...
				if( isset( $map[ 1 ] ) ){
					...
					if( $isRaw && in_array( $map[ 1 ], ['Object', 'JSON'] ) ){ continue; }
					...
					switch( $map[ 1 ] ){
						...
						case 'Object':
							$stack[ $col_key ] = unserialize( $result );
						break;

						case 'JSON':
							$stack[ $col_key ] = json_decode( $result, true );
						break;

						case 'String':
							$stack[ $col_key ] = $result;
						break;
				
						case 'Number':
							$stack[ $col_key ] = (double) $result;
						break;

						case 'Int':
							$stack[ $col_key ] = (int) $result;
						break;

						case 'Bool':
							$stack[ $col_key ] = (bool) $result;
						break;
	...
	public function select( $table, $join, $columns = null, $where = null ){
		...
		if( $columns === '*' ){ return $query->fetchAll( PDO::FETCH_ASSOC ); }

		if( $is_single ){ return $query->fetchAll( PDO::FETCH_COLUMN ); }
	...
	public function insert( $table, $datas ){
		...
		foreach( $datas as $data ){
			...
			foreach( $data as $key => $value ){ $columns[] = $key; }
...
//preg_replace("/(\s*\[(JSON|\+|\-|\*|\/)\]$)/i", '', $k)

			if( $raw = $this->buildRaw( $value, $map ) ){ $fields[] = $column . ' = ' . $raw;
...
*/					case 'array':
	...
	public function delete($table, $where){		$map = [];
...
public function delete($table, $where){	
	$m = [];
	return $this->exec('DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where, $m), $m);
...
*/
	...
	{
		if( !is_array( $columns ) || empty( $columns ) ){	return false;	}
	...
	public function has( $table, $join, $where = null ){
		...
		if( $query ){	$result = $query->fetchColumn();
	...
	public function action($actions){
		...
		if ( is_callable( $actions ) ){
			...
			try{	$result = $actions($this);

				if( $result === false ){	$this->pdo->rollBack();
...
$database = new _profane_abstraction_( $config["db"] );
